...
- **Dotted TLD/suffix fragments** — a token like `.ai`, `.app`, `.local`, or `.env`, split off as its own "word" right after a domain or hostname, is switched to English immediately rather than falling through the short-word dictionary and bigram model, neither of which recognizes a bare suffix.
...
- **Atomic config persistence** — settings are written by a background worker (`rswitcher-persist`) that coalesces burst saves and uses a write-to-temp-then-rename strategy, guaranteeing `config.json` is never partially written even under concurrent saves from the hook thread and IPC commands. A final synchronous save also runs on `RunEvent::Exit`, so learned corrections survive a session logoff/shutdown even when the app wasn't closed via the tray's Quit item.
