...
- **Short-word dictionary** — dedicated 2–3 character dictionaries (including terms like `usb`, `box`, `ok`, `wc`, `git`, `rl`, `ats`, `тв`, `чер`) cover short words that bigrams cannot score reliably.
- **Trailing Punctuation Trimming** — bigram and trigram scoring models trim trailing punctuation (such as commas, periods, quotes) from the end of the word before scoring, preventing punctuation from penalizing the word score and causing false negatives or false positives (e.g. for `vps,` or `info.`).
...
9. **Bigram+trigram scoring** — for words of 4+ characters (5+ for on-the-fly), each candidate is scored with a per-transition weighted log-probability:
   ```
